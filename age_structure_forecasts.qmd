...
census_science <- census_science |>
  mutate(
    age_group = fct_relevel(age_group, "100+", after = Inf),
...
ggplot(census_participation |> filter(Year %in% c("Average", 2016, 2021)), 
       aes(x = as.numeric(gsub("\\+", "", age_group)), y = participation, 
...
industry_retirement <- bind_rows(
  retirement_data |> mutate(industry = "Professional, scientific and technical services"),
  industry_retirement  
)

ggplot(industry_retirement, aes(x = age_group, y = pc)) +
  geom_col(alpha=0.6) +
...
all_retirement |> 
  ggplot(aes(x = Age, y = pc, group = industry)) +
  geom_line() +
  facet_wrap(~industry, ncol=2) +
  labs(
    y = "Percentage of retirement intentions",
