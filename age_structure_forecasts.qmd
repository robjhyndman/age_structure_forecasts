...
To forecast future working population numbers, $P_{i,x,t}$, we use the demographic growth-balance equation (@eq-dgb), with forecasts of the components $G_{i,t}$, $q_{x,t}$, and $E_{i,x,t}$. To account for the combined uncertainty from all of the forecast components, we use a simulation-based approach, running 1000 simulations to generate a forecast distribution of possible future outcomes.
...
We use the demographic growth-balance model @eq-dgb to simulate future populations, using the models described above for graduate completions, death probabilities and the remander terms. The following steps outline the process:

A total of 1000 simulations are run to obtain a distribution of possible future population scenarios.
The average of the 1000 simulations provides the mean age-specific forecast, while quantiles estimate forecast uncertainty. @fig-forecast-working-population presents the mean and 90% prediction intervals for 2025 and 2035.
