...
To use @eq-dgb, data on $P$, $D$, $R$, and $G$ is required, and the remainder component $E$ will be calculated using @eq-dgb. It is unlikely that we have available separate death and retirement counts for each group, and retirement data is not available in all years. So we will instead use probabilities of death and retirement across all groups, and assume retirement probability does not change over time, giving $D_{i,x,t} = P_{i,x,t}q_{x,t}$ and $R_{i,x,t} = P_{i,x,t}r_{x}$ where $q_{x,t}$ is the probability of death at each age $x$ in year $t$, and $r_{x}$ is the probability of retirement at each age $x$. Similarly, graduation numbers are rarely available by discipline and age, so we will approximate $G_{i,x,t} = g_xG_t$ where $G_{i,t}$ is the total number of graduates in year $t$ and $g_x$ is the proportion of graduates by age across all disciplines. So our more realistic model becomes
$$
P_{i,x+1,t+1} = P_{i,x,t}(1 - q_{x,t} - r_{x}) + g_xG_{i,t} + E_{i,x,t},
$$ {#eq-dgb2}
...
The demographic growth-balance equation (@eq-dgb2), when rearranged, provides an estimate of the remainder, including net migration and career changes:
$$
E_{i,x,t} = P_{i,x+1,t+1} - P_{i,x,t}(1 - q_{x,t} - r_{x}) - g_xG_{i,t},
$$ {#eq-remainder}
These remainders are shown in @fig-remainder.

```{r}
#| label: fig-remainder
#| fig-cap: Estimated remainder $E_{i,x,t}$ by discipline, age and year (2006--2020), calculated using the demographic growth-balance equation.
tar_read(fig22)
```
