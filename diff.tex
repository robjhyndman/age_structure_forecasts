...
%DIF DEL asf1.tex   Sun Nov 23 16:24:12 2025
%DIF ADD asf2.tex   Sun Nov 23 16:24:12 2025
% Options for packages loaded elsewhere
...
and immigration. We \DIFdelbegin \DIFdel{modify the traditional demographic growth-balance
equation to account for
workforce-specific dynamics}\DIFdelend \DIFaddbegin \DIFadd{propose a new dynamic statistical model for
forecasting the age structure of a workforce. Our approach is inspired
by a stochastic model used in population forecasting}\DIFaddend , replacing births
...
\DIFaddbegin \urlstyle{tt}

\DIFaddend \setstretch{1.5}
...
We propose \DIFdelbegin \DIFdel{to adapt this framework to }\DIFdelend \DIFaddbegin \DIFadd{a related approach for modelling }\DIFaddend workforce dynamics by
...
at any age. Second, we \DIFdelbegin \DIFdel{allow workers to leave }\DIFdelend \DIFaddbegin \emph{\DIFadd{explicitly}} \DIFadd{model workers leaving }\DIFaddend the
...
we model them \DIFdelbegin \DIFdel{implicitly }\DIFdelend \DIFaddbegin \emph{\DIFadd{implicitly}} \DIFaddend via a remainder term.
...
emigration, \DIFaddbegin \DIFadd{family responsibilities, }\DIFaddend or career change; they can enter
the workforce through graduation, immigration, \DIFaddbegin \DIFadd{changes in family
responsibilities, }\DIFaddend or career change. Unfortunately, we typically do not
have data on many of these processes, so we will combine \DIFdelbegin \DIFdel{career
change}\DIFdelend \DIFaddbegin \DIFadd{changes due to
family responsibilities, career changes}\DIFaddend , emigration and immigration into
...
\(t\). Then population changes can be described using \DIFdelbegin \DIFdel{a model similar to the stochastic
population modelof \mbox{%DIFAUXCMD
\citet{HB08}}\hskip0pt%DIFAUXCMD
, excluding the birth process}\DIFdelend \DIFaddbegin \DIFadd{the following
model}\DIFaddend : \begin{equation}\phantomsection\label{eq-dgb}{
...
\DIFaddbegin \DIFadd{While our model was inspired by the stochastic population model of
\mbox{%DIFAUXCMD
\citet{HB08}}\hskip0pt%DIFAUXCMD
, that model has different inputs (births and immigration)
...
\DIFaddend As a first approximation, the components \(q\), \(r\), \(E\) and \(G\)
...
\DIFaddbegin \DIFadd{Because the \(\beta_{k,t}\) values are principal component scores, they
...
accurate forecasts \mbox{%DIFAUXCMD
\citep[see,][]{HU07} }\hskip0pt%DIFAUXCMD
except in contrived simulated
examples. On the other hand, \mbox{%DIFAUXCMD
\citet{Aue2015} }\hskip0pt%DIFAUXCMD
did use multivariate models
to capture these cross-correlations, although they did not compare them
on real data.
}

\DIFaddend Functional data models have been widely used in demography and other
...
for the \(E_{i,x,t}\) scores\DIFaddbegin \DIFadd{, each estimated using maximum likelihood
estimation}\DIFaddend . The assumption of stationarity for the \(E_{i,x,t}\) scores
is validated for the disciplines we consider\DIFaddbegin \DIFadd{, but is not a requirement
...
\DIFaddbegin \DIFadd{This is estimated using least squares estimation. }\DIFaddend The global model pools
...
\DIFaddbegin \DIFadd{Fortunately, there is no reason to think scientists of different
...
\DIFadd{There has been a small increase in average retirement age over the last
ten years due to an increase in the age at which the old age pension can
be accessed \mbox{%DIFAUXCMD
\citep{pension}}\hskip0pt%DIFAUXCMD
, and a steady increase in the preservation
age at which superannuation can be accessed
\mbox{%DIFAUXCMD
\citep{kingston2019superannuation}}\hskip0pt%DIFAUXCMD
. However, there is no existing policy
...
\DIFadd{We assume the age distribution of graduates is a product of
...
\DIFadd{It is also worth pointing out that the remainder term \(E_{i,x,t}\) will
...
\DIFaddend \section{Data}\label{sec-data}
...
model \citep{HU07}, with \DIFdelbegin \DIFdel{stationary ARMA }\DIFdelend \DIFaddbegin \DIFadd{ARIMA }\DIFaddend models fitted to the principal component
scores. \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{In this case, all }\DIFaddend scores were found to be stationary using the
KPSS test \citep{KPSS}\DIFaddbegin \DIFadd{, so ARMA models are used}\DIFaddend . The forecasts for one
